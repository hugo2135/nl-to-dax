...
取代先前分開呼叫的 check_python_env.py / check_update.py / bookmarks.py list
——三次 subprocess 併為一次，SKILL.md 也只需描述一組指令。
各腳本仍可獨立執行，本檔只是薄薄的編排層，不複製任何邏輯。

此分支沒有本機設定檢查（認證走 MCP connector，由 SKILL.md 呼叫 `list_models`
驗證），因此不含 setup 區段。
